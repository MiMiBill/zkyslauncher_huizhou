...
public class OkGoUtil {
    ...
    public static void initOkGo(Application application) {
        ...
        if (!TextUtils.isEmpty(ActiveUtils.getRequestHeader())) {
            headers.put("PAD", ActiveUtils.getRequestHeader());
        }
//        if (!TextUtils.isEmpty(ActiveUtils.getRequestHeader())) {
...
//        params.put("commonParamsKey2", "这里支持中文参数");
        ...
        builder.connectTimeout(OkGo.DEFAULT_MILLISECONDS * 2, TimeUnit.MILLISECONDS);   //加长了全局超时时间 1分钟
...
//        builder.cookieJar(new CookieJarImpl(new DBCookieStore(this)));              //使用数据库保持cookie，如果cookie不过期，则一直有效
        ...
        OkGo.getInstance().init(application)                           //必须调用初始化
                ...
                .setRetryCount(0)    //不需要重连           //全局统一超时重连次数，默认为三次，那么最差的情况会请求4次(一次原始请求，三次重连请求)，不需要可以设置为0
