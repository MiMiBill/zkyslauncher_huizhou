package com.muju.note.launcher.topics;

public class SpTopics {

    /**
     *  本地影视数据更新时间
     */
    public static final String SP_VIDEO_UPDATE_TIME="sp_video_update_time";

    /**
     * 手动重启状态
     */
    public static final String SP_REBOOT="sp_reboot_phone";

    /**
     *  每日数据更新时间
     */
    public static final String SP_UPDATE_DATA_DAY="sp_update_data_day";

    /**
     *  每日上传数据时间
     */
    public static final String SP_UPLOAD_DATA_DAY="sp_upload_data_day";

    /**
     *  推送
     */
    public static final String SP_ZKY_PAD_LAUNCHER = "sp_zky_pad_launcher";
    public static final String SP_PAD_HAS_ACTIVE_ID = "sp_pad_has_active_id";
    public static final String SP_PAD_JPUSH_ID = "sp_pad_jpush_id";


    /**
     *  音量推送
     */
    public static final String PAD_CONFIG_VOLUME_RATE = "PadConfigEntityVolume";
    public static final String PAD_LOCAL_VOLUME = "PadLocalVolume";


}
